...
class SettingsController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        try {
            ...
            return redirect()->back()
                           ...
                           ->with('info', 'Please refresh the page to see logo changes.');
    ...
    private function buildDynamicCss()
    {
        ...
        // Colors
        if (isset($settings['theme.primary_color'])) {
            $css .= "    --primary-color: {$settings['theme.primary_color']->getProcessedValue()};\n";
        }
        if (isset($settings['theme.secondary_color'])) {
            $css .= "    --secondary-color: {$settings['theme.secondary_color']->getProcessedValue()};\n";
        }
        if (isset($settings['theme.success_color'])) {
            $css .= "    --success-color: {$settings['theme.success_color']->getProcessedValue()};\n";
        }
        if (isset($settings['theme.warning_color'])) {
            $css .= "    --warning-color: {$settings['theme.warning_color']->getProcessedValue()};\n";
        }
        if (isset($settings['theme.danger_color'])) {
            $css .= "    --danger-color: {$settings['theme.danger_color']->getProcessedValue()};\n";
        }
        
        // Fonts
        if (isset($settings['theme.font_family'])) {
            $css .= "    --font-family: {$settings['theme.font_family']->getProcessedValue()};\n";
        }
        if (isset($settings['theme.font_size'])) {
            $css .= "    --font-size: {$settings['theme.font_size']->getProcessedValue()}px;\n";
        }
        ...
        // Background image
        if (isset($settings['appearance.background_image']) && $settings['appearance.background_image']->getProcessedValue()) {
            $opacity = $settings['appearance.background_opacity']->getProcessedValue() ?? 0.1;
            ...
            $css .= "    width: 100%;\n";
            $css .= "    height: 100%;\n";
            $css .= "    background-image: url('{$settings['appearance.background_image']->getProcessedValue()}');\n";
